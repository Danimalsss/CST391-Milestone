...
export const updateAlbum: RequestHandler = async (req: Request, res: Response) => {
    console.log("Update Album Function Called");
    const albumId = Number(req.params.albumId);  // Get the album ID from the route parameter
    console.log(`Updating album with ID: ${albumId}`);
    try {
        // Update the album using the album ID and the request body data
        const okPacket: OkPacket = await AlbumDao.updateAlbum(albumId, req.body);

        console.log('req.body', req.body);
        console.log('album', okPacket);

        // Optionally update associated tracks if provided in the request
        if (req.body.tracks) {
            await Promise.all(
        ...
        console.error('[albums.controller][updateAlbum][Error] ', error);
        res.status(500).json({
            message: 'There was an error when updating the album'
        });
    }
};
